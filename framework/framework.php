<?php
/**
 * Squaretrix Framework - Inspired by Yoast's Plugins and WooThemes Framework
 *
 * @package WordPress
 * @subpackage Terbit_Coming_Soon_Page
 * @since 0.1
 */
if (!class_exists('squaretrix_Framework')) {
	class squaretrix_Framework {
        ...
        /**
         * Global we set in squaretrix_admin_enqueue_scripts and use in create_menu
         */
        public $pages = array();
        ...
        function admin_enqueue_scripts( $hook_suffix ) {
            wp_enqueue_style( 'squaretrix_mm_plugin', plugins_url('inc/css/admin-style.css',dirname(__FILE__)), false, $this->plugin_version );
            if(!in_array($hook_suffix, $this->pages))
                return;
            wp_enqueue_script('dashboard');
        	wp_enqueue_script( 'squaretrix_framework', plugins_url('framework.js',__FILE__), array( 'jquery','media-upload','thickbox','farbtastic' ), $this->plugin_version );
        	wp_enqueue_style( 'squaretrix_framework', plugins_url('framework.css',__FILE__), false, $this->plugin_version );
        	wp_enqueue_script( 'squaretrix_plugin', plugins_url('inc/js/admin-script.js',dirname(__FILE__)), array( 'jquery','media-upload','thickbox','farbtastic' ), $this->plugin_version );
        	wp_enqueue_style( 'squaretrix_plugin', plugins_url('inc/css/admin-style.css',dirname(__FILE__)), false, $this->plugin_version );
        ...
        function option_page() {
            $page = $_REQUEST['page'];
        	?>
        	<div class="wrap squaretrix">
        	    <?php screen_icon(); ?>
        		<h2><?php echo $this->plugin_name; ?> </h2>
                      		<?php //settings_errors(); ?> 
                <h2 class="nav-tab-wrapper" style="padding-left:20px">
                    <a class="nav-tab seed_csp3-support" href="options-general.php?page=squaretrix_coming_soon"><?php _e('Settings','terbit-coming-soon') ?></a>
                    <a class="nav-tab seed_csp3-preview thickbox-preview" href="<?php echo home_url(); ?>?cs_preview=true&TB_iframe=true&width=640&height=632" title="<?php _e('&larr; Close Window','terbit-coming-soon') ?>"><?php _e('Live Preview','terbit-coming-soon'); ?></a>
                </h2>
        		<div id="poststuff" class="metabox-holder">
                    <!--<div id="side-info-column" class="inner-sidebar">
                        <div id="side-sortables" class="meta-box-sortables ui-sortable">
					     	<a href="http://www.squaretrix.com/plugins/wordpress-coming-soon-pro-plugin/?utm_source=plugin&utm_medium=banner&utm_campaign=coming-soon-pro-in-plugin-banner" target="_blank"><img src="http://static.squaretrix.com/ads/coming-soon-pro-sidebar.png" /></a>
                            <br><br>
                            <div class="postbox support-postbox">
                                <div class="handlediv" title="Click to toggle"><br /></div>
                				<h3 class="hndle"><span><?php _e('Plugin Support', 'terbit-coming-soon') ?></span></h3>
                				<div class="inside">
                					...
                					<ul>
                					    <li>&raquo; <a href="<?php echo (empty($this->plugin_support_url) ? 'http://squaretrix.com/support/' : $this->plugin_support_url) ?>" target="_blank"><?php _e('Support Request', 'terbit-coming-soon') ?></a></li>
                				    </ul>
                            ...
                            <div class="postbox rss-postbox">
                                <div class="handlediv" title="Click to toggle"><br /></div>
                				<h3 class="hndle"><span><?php _e('squaretrix Blog', 'terbit-coming-soon') ?></span></h3>
                				<div class="inside">
                					<div class="rss-widget">
                					<?php
                					wp_widget_rss_output(array(
                					   'url' => 'http://squaretrix.com/feed/',
                					   'title' => 'squaretrix Blog',
                					   'items' => 3,
                					   'show_summary' => 0,
                					   'show_author' => 0,
                					   'show_date' => 1,
                					...
                					?>
            					    <ul>
                					    <li>&raquo; <a href="http://squaretrix.com/subscribe/"><?php _e('Subscribe by Email', 'terbit-coming-soon') ?></a></li>
                				    </ul>
                					</div>
                				</div>
                            </div>
                                        ...
                                        if($v['menu_slug'] == $page){
                                            switch ($v['type']) {
                                                case 'setting':
                            				        ...
                            				        break;
                            				    case 'section':
                            				        echo '<div class="postbox squaretrix-postbox"><div class="handlediv" title="Click to toggle"><br /></div>';
                                            		$this->squaretrix_do_settings_sections($v['id']);
                                        		    echo '</div>';
                                        		    break;
        ...
        function field_machine($args) {
        	...
        	switch($type){
                ...
                case 'image':
        	        ...
        	        <input id='{$id}_upload_image_button' class='button-secondary upload-button' type='button' value='". __('Media Image Library', 'terbit-coming-soon')."' />
        	        <br><small class='description'>".(empty($desc) ? '' : $desc)."</small>
        	        ";
        	        break;
        	    case 'select':
            ...
            if(!$error){
				...
				if(!empty($wp_settings_errors)){
					foreach($wp_settings_errors as $k=>$v){
						if($v['code'] == 'squaretrix_settings_updated')
							$display = false;
					}
				}
				if($display)
		        	add_settings_error('general', 'squaretrix_settings_updated', sprintf(__("Settings saved.  <a href='%s/?cs_preview=true'>Preview &raquo;</a>", 'terbit-coming-soon'),home_url()), 'updated');
            }
        	return $input;
        }
         ...
         */
         function font_field_list($show_google_fonts = true){
              $fonts = unserialize(get_transient('squaretrix_fonts'));
              if($fonts === false){
                 ...
                 if(!empty($google_fonts)){
                     set_transient('squaretrix_fonts',serialize( $fonts ),86400);
                }
             }
             return $fonts;
         }
         ...
         /**
          * squaretrix version of WP's do_settings_sections
          *
          * @since 0.1
          */
         function squaretrix_do_settings_sections($page) {
             ...
             foreach ( (array) $wp_settings_sections[$page] as $section ) {
                 ...
                 echo '</table>';
                 echo '<p>';
                 echo "<input name=\"Submit\" type=\"submit\" value=\"". __('Save Changes', 'terbit-coming-soon') ."\" class=\"button-primary\"/>";
                 echo '</p>';
                 echo '</div>';
             }
         }
